...
Samples get the same treatment (beeline-ewl, the second instance of [ADR 0007](docs/adr/0007-authored-changes-are-events.md)): `data/sample-change.csv` (`src/sample-change.ts`) is append-only events, produced by the same compare-against-what-was-last-said pass at the end of legacy promotion, after observation promotion, and at boot — so the nightly's coordinate rewrites, minting, free-linking, and a staff edit all leave a trace, and a writer that forgets is caught by the next pass. The edit screen runs a pass narrowed to its one sample, so the author is credited with their edit and nothing else. Where it deliberately differs from the person log: the baseline lives in `data/sample-state.csv` — one current row per sample, restated wholesale each pass — because the person log's in-log baseline over this corpus would be a million rows; the reference is the derived triple (collector as the overlay names them, number, start date), with the observation link recognising a sample across a corrected number or date and a triple collision (a live duplicate) recorded as nothing and counted. The record page reads it as the History panel on `/samples/:id`. Measured on the dev store: baseline 1.1 s and 11.2 MB, a steady pass ~2 s and zero entries, and a nightly-shaped 30 moves + 2 renumbers recording exactly 32.
