...
## Printing and mailing (1-3 and 6-7 to Arthur; 4-5 are Peter and Andony's to decide)

Arthur answered the proofing, stock, label-lifecycle and number-scrapping questions on 2026-09-08; those are recorded in [CONTEXT.md](../CONTEXT.md) (Voided field number) and [reference-implementation.md](reference-implementation.md) (The printing moment, as the operator runs it) and are gone from here. What is left is what he did not answer, plus what his answers newly opened — and it is no longer one audience. Questions 1-3 and 6-7 are Arthur's, being about what happens at the printer. Questions 4 and 5 are not questions for him at all: he has told us the practice, and what to do about it is a decision for Peter and Andony.
...
5. **Should Beeline keep the renumbering rule at all?** Arthur described current practice and sketched a new-Beeline version of it (reprint under a new number, keep a record, and use a flag reference sheet to pull those specimens for manual verification when the bees come in). The project's provisional stance was the opposite — the number is the specimen's permanent identity, a data-error reprint carries it, and minting is reserved for identity errors ([beeline-1kb.5](../.beads/)). One of the two has to go, and the choice decides whether voiding needs first-class events. **Arthur's is the practice staff have; ours is a proposal — so this is a decision for Peter and Andony, not a question for Arthur.**
6. **If a number is retired after publication, how do Ecdysis and GBIF learn of it?** Unanswered, and sharper now: if a data-error reprint renumbers, retirement is routine rather than exceptional, and the museum's catalog number is derived from the number that was retired.
7. **What does the `E` prefix on 2020–2022 Oregon field numbers** (`E2000000`–`E2332481`, 1,400 records) denote? And the 2018 name-based identifiers (`First_Last:18.sss.nnn`) — are those printed verbatim on physical labels?
8. **Where did the historical cross-project duplicate field numbers live** — this database, Ecdysis, or between them — and were they renumbered, or do the physical labels still collide?
