...
describe("an unpacked Darwin Core archive", () => {
  test("records the history, and the identification Ecdysis calls current is the record", async () => {
    ...
    expect(result.determiners).toEqual([
      { name: "Ellen Expert", rows: 3, resolved: false },
      { name: "Sam A. Staff", rows: 3, resolved: true },
    ]);
    ...
    expect(await rows(conn, `SELECT record_id, occurrence_id, CAST(entered_at AS VARCHAR) FROM ecdysis_identification ORDER BY record_id`)).toEqual([
      ...
      ["id-d1", "occ-d", "2026-02-03 17:32:21"],
      ["id-d2", "occ-d", "2026-02-20 11:00:00"],
    ]);
  });
  ...
  test("is idempotent: the same archive again records nothing", async () => {
    ...
    expect(again).toMatchObject({ loaded: 0, alreadyLoaded: 6, unresolvedNames: [{ name: "Nomada", rows: 1 }] });
    expect(await one(`SELECT count(*) FROM determination`)).toEqual([6n]);
  });
  ...
  test("a wrong prefix matches nothing and says so", async () => {
    ...
    expect(result).toMatchObject({ matched: 0, unmatched: 6, loaded: 0 });
  });
});
...
describe("a flat occurrence export", () => {
  test("re-stating the archive's current identification records nothing; a revised one supersedes it", async () => {
    ...
    expect(same).toMatchObject({ input: "flat", occurrences: 2, matched: 2, identifications: 2, alreadyLoaded: 2, loaded: 0 });
  ...
  test("an archive loaded after a flat export records its history and restates nothing", async () => {
    ...
    expect(await one(`SELECT count(*) FROM determination`)).toEqual([2n]);
    const archive = await loadEcdysis(conn, { path: `${FIXTURES}archive`, catalogPrefix: "WSDA_", determinerAliases: ALIASES });
    // The archive's current rows for A and D are the flat load's events,
    // now under their recordIDs; the older rows are new history and land
    // before them in time.
    expect(archive).toMatchObject({ alreadyLoaded: 2, loaded: 4 });
    expect(await one(`SELECT count(*) FROM ecdysis_identification WHERE occurrence_id = 'occ-a'`)).toEqual([3n]);
    expect((await record(a))![0]).toBe("Andrena sladeni");
  ...
  test("a flat export's current row, later superseded in the archive, is adopted as that history rather than skipped or doubled", async () => {
    // The case the first version got wrong on real data: the flat export
    // says Ceratina by Sam; the archive says that was superseded by
    // Ceratina acantha. Skipping the superseded row because it matched the
    // record as it then stood left it for the next load to record.
    await loadEcdysis(conn, { path: `${FIXTURES}flat-d.csv`, catalogPrefix: "WSDA_", determinerAliases: ALIASES });
    const archive = await loadEcdysis(conn, { path: `${FIXTURES}archive`, catalogPrefix: "WSDA_", determinerAliases: ALIASES });
    expect(await record(d)).toEqual(["Ceratina acantha", null, "Ellen Expert", "2026-01-01", "year", null, "ecdysis_import"]);
    // Two events, not three: the archive's superseded row is the flat
    // load's event, now carrying its recordID as well.
    expect(await one(`SELECT count(*) FROM determination WHERE specimen_id = ${d}`)).toEqual([2n]);
    expect(await rows(conn, `SELECT record_id FROM ecdysis_identification WHERE occurrence_id = 'occ-d' ORDER BY record_id`)).toEqual([["id-d1"], ["id-d2"], [expect.stringMatching(/^occ:occ-d:/)]]);
    expect(await one(`SELECT count(DISTINCT determination_id) FROM ecdysis_identification WHERE occurrence_id = 'occ-d'`)).toEqual([2n]);
    // And the archive again records nothing.
    const again = await loadEcdysis(conn, { path: `${FIXTURES}archive`, catalogPrefix: "WSDA_", determinerAliases: ALIASES });
