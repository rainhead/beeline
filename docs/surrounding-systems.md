...
- **Ecdysis** — the entomological collections database; receives specimen records, returns expert determinations — both by hand for the time being, through each atlas's staff (2026-09-11). Note the fun bit: **GBIF receives our data both directly and via Ecdysis**, so identifier stability and record identity matter doubly (GBIF dedup is imperfect; see salishsea-io's SRC-01 rule for the same problem handled deliberately).
