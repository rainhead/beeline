...
8. **Does a qualifier print, and does it block?** A determination can now say `cf.`, `aff.` or `nr.` ([beeline-tgu](../.beads/)) — the determiner got to a species and stopped short of asserting it. A label prints one name. Does `Lasioglossum nr. tenax` print with the `nr.`, print as the bare species, print as the genus, or not print at all until someone confirms it? Only three legacy records are affected today, but the answer decides whether printability gains a clause, and it is the same shape as the taxon-geoprivacy question: what the store knows and what a label may say are different questions.
9. **Field-number lifecycle**: has a number ever been permanently scrapped/voided — specimen destroyed, label printed but never pinned, duplicate collision? If a number were retired after publication, how would Ecdysis/GBIF learn of it?
