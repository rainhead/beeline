...
2. **Field-number** uniqueness must be a hard guarantee, not a scan-and-increment convention — but it is a guarantee over the numbers **Beeline mints**, not over the column. A bare `UNIQUE` on `specimen.field_number` cannot be created at all: `25051768` arrives on two imported rows, and the partial unique index that would exclude them is Postgres-only and forbidden by [ADR 0001](adr/0001-duckdb-first-with-portable-sql.md). The mechanism is the `minted_field_number` registry ([schema-sketch.md](schema-sketch.md), and decision 5 of [ADR 0008](adr/0008-specimen-identity.md)): the insert *is* the mint and its PRIMARY KEY is the guarantee — among minted numbers. Not colliding with the 383,031 imported ones is a separate obligation the engine cannot express portably, met by taking the seed from the imported corpus as well as the registry, and by the registry outliving a rebuild at all — it is authored data, and a number burned by a cancelled run is in no other source — then checked by a view asserted empty (decision 5 of [ADR 0008](adr/0008-specimen-identity.md)). Legacy numbers stay attributes on the specimen, governed by nothing. Stated as "catalog number" until 2026-09-09, which was this project's own confusion rather than a claim about the museum's identifier — the two are different things (beeline-nfo, and *Catalog number* in [CONTEXT.md](../CONTEXT.md)); the finding above it is about `fieldNumber` and always was.
