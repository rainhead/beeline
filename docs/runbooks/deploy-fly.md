...
[`infra/fly/entrypoint.sh`](../fly/entrypoint.sh). **Temporary hosting** in
...
there is no window for a second process. A failed migration fails the boot,
the health check never passes, and Fly keeps the previous release.
...
Then deploy, and fill the volume. The store must be a **reseeded** one —
check with `pnpm db:migrate --check` first, because a store shaped before
beeline-6e9 cannot be migrated forward and will boot into 500s on every
listing ([ADR 0006](../adr/0006-migrations-for-deployed-stores.md)).

```sh
fly deploy
fly ssh sftp shell --app beeline     # put the store and the change-log CSVs
```
